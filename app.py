...
# 信用卡選項與結帳日設定 (Key: 選單顯示名稱, Value: 結帳日)
CREDIT_CARDS = {
    "現金": 0,
    "聯邦 (結帳19日)": 19,
    "兆豐-linepay (結帳5日)": 5,
...
def add_transaction(date_obj, record_type, category, amount, payment_method, note):
    ...
    # 寫入資料：date, type, category, amount, payment_method, note, id
...
if not df.empty and 'id' in df.columns:
    ...
    for index, row in delete_df.iterrows():
        ...
        pm_short = row.get('payment_method', '')[:2] # 簡顯示
...
if not df.empty:
    ...
    with c1:
        ...
        if not expense_df.empty:
            ...
            fig_pie = px.pie(pay_stats, values='amount', names='payment_method', title='錢都花哪張卡？', hole=0.4)
    ...
    with c2:
        ...
        if not expense_df.empty:
            fig_bar = px.bar(expense_df, x='category', y='amount', color='payment_method', title='各類別花費與支付方式')
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            "id": None, 
            "_sheet_name": None,
            "billing_cycle": st.column_config.TextColumn("帳單歸屬 (自動推算)", disabled=True), # 此欄位唯讀
